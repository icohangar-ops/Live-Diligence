...
export const exportWebhookEvents = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: {
    eventType?: string;
    payloadStyle?: string;
  ...
  .handler(async ({ data, context }) => {
    ...
    let query = supabase
      .from("webhook_events")
      .select("created_at, source, event_id, event_type, payload_style, verified, status, error")
      .order("created_at", { ascending: false });
...
export const exportWebhookEventsJSON = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: {
    eventType?: string;
    payloadStyle?: string;
  ...
  .handler(async ({ data, context }) => {
    ...
    let query = supabase
      .from("webhook_events")
      .select("created_at, source, event_id, event_type, payload_style, verified, status, error, payload")
      .order("created_at", { ascending: false });
